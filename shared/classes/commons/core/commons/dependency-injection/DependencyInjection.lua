...
function DependencyInjection:produce(scope, name, ...)
    Log.trace("Producing " .. name .. " in scope " .. scope)
...
function DependencyInjection:inject(name, attr, ...)
    Log.trace("Injecting " .. name .. " in scope on attr " .. attr, self.classesScope[name])
...
function DependencyInjection:registerInjectableCandidat(className, scope)
    Interface.thread(function()
        ...
        Log.trace("Registered " .. className .. " in scope " .. scope)
    end)
end
